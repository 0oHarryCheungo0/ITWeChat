...
?>
<fieldset class="layui-elem-field layui-field-title" style="padding-bottom:50px">
    <legend>微信轮播图</legend>
...
<script type="text/javascript">
    ...
    var _AJAXURL__ = "<?=Url::to(['system-rules/slider-data'])?>";
    ...
    window.operateEvents = {
        'click .edit': function (e, value, row, index) {
            url = "<?=Url::to(['system-rules/slider-edit'])?>" + "?slider_id=" + row.id + '&table_index=' + index;
            layer.open({
                type: 2,
                title: '编辑',
                area: ['500px', '80%'],
        ...
        'click .delete': function (e, value, row, index) {
            layer.confirm('确认删除轮播图？', {icon: 3, title: '确认删除？', shadeClose: true,}, function (layer_index) {
                url = "<?=Url::to(['vips/sure-to-send'])?>";
                $.post("<?=Url::to(['system-rules/delete'])?>", {slider_id: row.id}, function (ret) {
                    if (ret.code == 0) {
                        layer.msg('删除成功');
    ...
    $(".source-add").click(function () {
        layer.open({
            ...
            shadeClose: true,
            isOutAnim: false,
            content: '<?=Url::to(['system-rules/slider-add'])?>' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    })
    $('.wxset').show();
    $('#menu_slider').attr('class', 'active');
</script>
