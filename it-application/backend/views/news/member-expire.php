...
<form class="layui-form" action="">
    <div class="layui-form-item">
        <label class="layui-form-label">选择到期月份</label>
        <div class="layui-input-inline">
            <select name="type_children" lay-verify="required">
                <option value=""></option>
                <?php foreach ($month as $k => $v): ?>
                    <option value="<?= $k ?>"
                            <?php if ($k == $data->type_children): ?>selected<?php endif; ?>><?= $v ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" name="end_time" id='end_time' onclick="laydate({elem:this,format: 'YYYY-MM-DD hh:mm:ss',istime: true})" class="layui-input"
                   value="<?php if ($data->end_time != 0) {
                       echo date('Y-m-d', $data->end_time);
                   } ?>">
        </div>
    </div>
...
<script type="text/javascript">
    ...
    var UPDAURL = '<?=Url::toRoute("news/update-data")?>';
    ...
    layui.use(['form', 'laydate'], function () {
        ...
        form.on('submit(formDemo)', function (data) {
            ...
            $.post(UPDAURL, {data: post_data}, function (data) {
                if (data.code == 200) {
                    layer.msg('成功', {icon: 1});
                    location.href = "<?=Url::toRoute('news/expire-list', true)?>";
                } else {
