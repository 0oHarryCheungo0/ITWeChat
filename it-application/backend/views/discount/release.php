    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" name="end" id='end_time' onclick="laydate({elem:this,format: 'YYYY-MM-DD hh:mm:ss',istime: true})" class="layui-input"
                   value="<?= $data->end!=0?date('Y-m-d', $data->end):'' ?>">
        </div>
    </div>
     <div class="layui-form-item">
        <label class="layui-form-label">定时发布时间</label>
        <div class="layui-input-inline">
            <input type="text" name="send_time" id='end_time' onclick="laydate({elem:this,format: 'YYYY-MM-DD hh:mm:ss',istime: true})" class="layui-input"
                   value="<?= $data->end!=0?date('Y-m-d', $data->send_time):'' ?>">
        </div>
    </div>
...
<script type="text/javascript">
    ...
    var UPDAURL = '<?=Url::toRoute("discount/update-limit")?>';
    ...
    layui.use(['form', 'laydate'], function () {
        ...
        form.on('submit(formDemo)', function (data) {
            ...
            $.post(UPDAURL, {data: post_data}, function (data) {
                if (data.code == 200) {
                    layer.msg('成功', {icon: 1});
                    location.href = "<?=Url::toRoute('discount/list', true)?>";
                } else {
