...
<a href="<?= Url::toRoute('news/add-birth-news') ?>"
   style="text-decoration:none;float:right;margin-top:20px;margin-left:12px;"
   class="layui-btn layui-btn-small layui-btn-warm" id="bar">新增生日月资讯</a>
...
<script type="text/javascript">
    ...
    function operateFormatter(value, row, index) {
        if (row.status ==1){
              return [
            '<button class="set layui-btn  layui-btn-mini "><i class="layui-icon">&#xe631;</i>设置</button><button class="scan layui-btn  layui-btn-mini "><i class="layui-icon">&#xe60a;</i>查看</button><button class=" layui-btn  layui-btn-mini  layui-btn-disabled "><i class="layui-icon">&#xe642;</i>编辑</button><button class="del layui-btn  layui-btn-mini layui-btn-danger"><i class="layui-icon">&#xe640;</i>删除</button>',
        ].join('');

        }else{
              return [
            '<button class="set layui-btn  layui-btn-mini "><i class="layui-icon">&#xe631;</i>设置</button><button class="scan layui-btn  layui-btn-mini "><i class="layui-icon">&#xe60a;</i>查看</button><button class="edit layui-btn  layui-btn-mini "><i class="layui-icon">&#xe642;</i>编辑</button><button class="del layui-btn  layui-btn-mini layui-btn-danger"><i class="layui-icon">&#xe640;</i>删除</button>',
        ].join('');
    ...
    layer.confirm('确定删除?', {
        ...
        $.post('<?=Url::toRoute('news/del-birth')?>?id=' + id,
            {id: id},
            function (data) {
                if (data.code == 200) {
    ...
    function edit(id) {
        location.href = "<?=Url::toRoute('news/edit-birth')?>?id=" + id;
    }
